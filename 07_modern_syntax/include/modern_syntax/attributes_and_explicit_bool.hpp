...
// ║  常见陷阱                                                                    ║
// ║                                                                            ║
// ║  陷阱1: [[no_unique_address]] 对同类型的两个空成员无效                     ║
// ║         编译器必须给同类型的两个对象不同的地址, 所以即使加了属性,           ║
// ║         两个同类型空成员也不能共享地址                                      ║
// ║                                                                            ║
// ║  陷阱2: MSVC 对 [[no_unique_address]] 的支持需要 /std:c++20 或更高       ║
// ║         (MSVC 19.28+ 支持; GCC 11+; Clang 13+ 均支持)                      ║
// ║                                                                            ║
// ║  陷阱3: explicit(bool) 中的表达式必须是编译期可求值的                      ║
// ║         不能用运行时变量, 必须依赖模板参数或 constexpr 值                   ║
// ║                                                                            ║
// ║  陷阱4: hardware_destructive_interference_size 是建议值, 非强制            ║
// ║         在某些平台上可能比实际 cache line 更大(保守估计),                   ║
// ║         也可能未定义 (很少见, 嵌入式环境需注意)                             ║
// ║                                                                            ║
// ║  陷阱5: 伪共享优化过度可能导致内存浪费                                      ║
// ║         如果只有少数线程访问, 强制对齐到 64/128 字节可能                     ║
// ║         得不偿失 —— 仅在 profiling 确认有伪共享后优化                       ║
// ╚══════════════════════════════════════════════════════════════════════════════╝

// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  练习任务                                                                    ║
// ║                                                                            ║
// ║  练习1: 创建一个带有多策略类的管理器, 用 [[no_unique_address]] 优化        ║
// ║         提示: 定义 Comparator, Allocator 两个空策略类,                      ║
// ║         然后用 Manager<Comparator, Allocator> 持有它们                      ║
// ║                                                                            ║
// ║  练习2: 为练习1的管理器添加条件 explicit 转换构造函数                      ║
// ║         提示: 使用 explicit(!std::is_convertible_v<U, T>)                   ║
// ║                                                                            ║
// ║  练习3: 设计一个线程安全的双计数器, 使用                                  ║
// ║         hardware_destructive_interference_size 避免伪共享                   ║
// ║         提示: 定义两个 atomic<int64_t>, 分别用 alignas 对齐                 ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
