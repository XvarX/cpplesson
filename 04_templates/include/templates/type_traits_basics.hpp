...
// 常见陷阱
// ═══════════════════════════════════════════════════════════════════════════════
// 1. enable_if 放在返回值类型、默认模板参数或非类型模板参数位置都可以
// 2. SFINAE 只在模板参数替换时生效, 函数体内的错误照常报错
// 3. void_t 可以检测表达式有效性, 不限于类型成员
// 4. if constexpr 的条件必须是编译期常量表达式
// 5. C++20 concepts 是更好的替代方案 (见下一课), 但理解 SFINAE 帮你读懂旧代码
